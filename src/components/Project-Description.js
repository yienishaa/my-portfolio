...
import { Container, Grid, Divider, Stack, List, ListItem, Typography, Box } from '@mui/material';
...
const projecttitle = [
  ...
  'Portfolio Website',
  'SMART ROAM',
  'Lost Roamer heat map',
  'Bill Shock Prevention Notifier',
];

const ecom_ov =
  'The clinic website was entirely a static HTML/CSS website with no shopping cart facility. The new features added included: ';
const ecom_list = [
  'shopping cart',
  'subscription plans for recurring purcherses',
  'PayPal payments (one time and subscription charging)',
  'appointment reminders via email and SMS (twilio)',
  'shipping label creation with Canada Post API',
  'Data collection of items added to cart by users',
];
const website_ov = 'This website was created using ReactJS, ChakraUI and NodeJS';
const smartroam_ov =
  'Developed a USSD menu to activate, deactivate Smartroam feature, to check roaming rates for each country. Used the Mylinex USSD gateway to integrate the system.';
const lostroam_ov =
...
function ProjectDescription({ id = 0 }) {
  return (
    <Container maxWidth="lg" sx={{ color: 'text.primary', py: 2 }}>
      <Typography variant="h5" sx={{ fontFamily: 'Yeseva One', mb: 1 }}>
        {projecttitle[id]}
      </Typography>
      <Divider sx={{ mb: 2 }} />
      ...
      <Grid container spacing={4}>
        <Grid item xs={12} md={6}>
          <Stack spacing={2}>
            ...
            <Typography sx={{ fontFamily: 'Montserrat', fontWeight: 400 }}>{overviews[id]}</Typography>
            <ListOfItems id={id} />
          </Stack>
        </Grid>
        ...
        <Grid item xs={12} md={6}>
          <Stack spacing={2}>
            ...
            <Typography sx={{ fontFamily: 'Montserrat', fontWeight: 400 }}>{status[id]}</Typography>

            <Typography variant="subtitle1" sx={{ fontWeight: 700, mt: 2 }}>
              SKILLS
            </Typography>
            <Typography sx={{ fontFamily: 'Montserrat', fontWeight: 400 }}>{skills[id]}</Typography>
          </Stack>
        </Grid>
      </Grid>
    </Container>
